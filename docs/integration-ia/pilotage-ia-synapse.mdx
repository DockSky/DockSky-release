...
1. **Compte DockSky App** avec Synapse activé (Free ou Pro)
2. Pour **`synapse_exec` via MCP** : plan **Pro / Équipage** (scope `mcp:synapse`)
3. **Token IA** généré dans Paramètres → Accès IA
4. **Au moins une machine** enregistrée avec des conteneurs autorisés
5. **Secrets** dans le coffre pour chaque placeholder utilisé dans les commandes

Sans compte Bandeau, les sections Pilotage IA restent désactivées.

---

## Parcours type (5 minutes)

1. Vérifie que **Synapse** est actif (compte Bandeau, voir [Tarifs](../concepts/tarifs-plans))
2. **Paramètres → Accès IA** : génère un token Pro si tu veux `synapse_exec` via MCP
3. **Paramètres → Coffre Pilotage IA** : enregistre `DB_USER`, `DB_PASS`, etc.
4. **Paramètres → Mes machines Synapse** : crée une machine (ex. slug `mon-vps`, conteneurs `mon_app`)
5. Configure **Cursor** ou **VS Code** avec le serveur MCP DockSky → voir [Synapse et MCP](./synapse-mcp-cursor)
6. Demande à ton IA : *« Liste les bases MySQL sur mon-vps (conteneur ma_base_mysql) avec le secret MYSQL_ROOT »*, voir [exemple détaillé](./synapse-mcp-cursor#exemple-concret--lister-les-bases-mysql)
...
| Scopes sur le token IA | Affichés dans Paramètres → Accès IA ; Pro inclut `mcp:synapse` |
| Plan gratuit | Synapse actif (quotas) ; `synapse_exec` **MCP** réservé au scope Pro ; exec REST possible dans les quotas |
