...
| Action | Free Bandeau | Pro Bandeau |
|--------|--------------|-------------|
| Voir les sections Paramètres | Oui | Oui |
| Créer / modifier secrets et machines (app ou API) | Oui (quotas) | Oui |
| Exécuter via API REST (`/synapse/exec`) | Oui (quotas) | Oui |
| Exécuter via MCP `synapse_exec` | Non (scope Pro) | Oui |
...
| Méthode | Chemin | Pro requis | Description |
|---------|--------|------------|-------------|
| `GET` | `/status` | Non | `{ enabled, plan_required, message }` |
| `GET` | `/secrets` | Non | Liste des noms |
| `PUT` | `/secrets/{name}` | Non *(quotas Free+)* | Corps : `{ "value": "…" }` |
| `DELETE` | `/secrets/{name}` | Non *(quotas Free+)* | Suppression |
| `GET` | `/machines` | Non | Liste des machines |
| `POST` | `/machines` | Non *(quotas Free+)* | Création |
| `PUT` | `/machines/{slug}` | Non *(quotas Free+)* | Mise à jour |
| `DELETE` | `/machines/{slug}` | Non *(quotas Free+)* | Suppression |
| `POST` | `/exec` | Non *(quotas Free+)* | Exécution shell |
