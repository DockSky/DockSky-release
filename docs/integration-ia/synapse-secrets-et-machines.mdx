...
Les sections **Coffre**, **Machines** et **Journal** sont actives uniquement avec un plan **Pro** ou **Personal** DockSky App.

| Action | Plan gratuit | Plan Pro / Personal |
|--------|--------------|---------------------|
...
```bash
docker exec {{CONTAINER}} mysql -u {{DB_USER}} -p{{DB_PASS}} -e "SHOW DATABASES"
```

Le serveur remplace `{{DB_USER}}` et `{{DB_PASS}}` **avant** l'exécution. L'IA ne voit jamais les valeurs réelles.
...
L'app crée toujours des machines de type `docker_local` (exécution Docker sur l'infra DockSky). Le type `agent` (VPS distant) est disponible via API/MCP — voir [Agent distant](./synapse-agent-distant).
:::
...
{
  "machine": "docksky-vps",
  "command": "docker exec ctn_mysql mysql -u {{DB_USER}} -p{{DB_PASS}} -e 'SHOW DATABASES'",
  "timeout_seconds": 30
}
```
